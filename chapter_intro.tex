...
\end{align*}
%
Perhaps the most important component in this Lagrangian are the gluon gauge fields, $\mathcal{A}_\mu^A$, with index $A$ representing one of the eight color labels for the gluon. These eight fields belong to the color group SU$_\text{c}$(3), which is the gauge group of the theory. 

The quarks are represented by the field $q$, which can be broken up in the following way:
%
\begin{enumerate}
    \item The $q$ field has six flavor components, $q(x) = \{u(x), d(x), s(x), c(x), b(x), t(x)\}$.
    \item Those flavor components have four relativistic spin components, with corresponding chiral components $q_{R,L} = \frac{1}{2}(1 \pm \gamma^5)q$
    \item Those spin components have three ``color'' components.
\end{enumerate}
%
